  ...
  _createClass(Prosperent, [{
    ...
    value: function usProducts() {
      ...
      if (Object.keys(options).map(function (v) {
        return options[v].include && options[v].includes('&');
      ...
      return _axios2.default.get('http://api.prosperent.com/api/search?api_key=' + this.apiKey + '&location=' + this.location + ifTrue(this.clickMaskDomain, '&clickMaskDomain=' + this.clickMaskDomain) + ifTrue(query, '&query=' + query) + ifTrue(this.imageMaskDomain, '&imageMaskDomain=' + this.imageMaskDomain) + ifTrue(filterBrand, '&filterBrand=' + convertArrayToString(filterBrand)) + ifTrue(filterPrice, '&filterPrice=' + floatOrRange(filterPrice)) + ifTrue(filterPriceSale, '&filterPriceSale=' + floatOrRange(filterPriceSale)) + ifTrue(filterPercentOff, '&filterPercentOff=' + floatOrRange(filterPercentOff)) + ifTrue(sortBy, '&sortBy=' + sortBy) + ifTrue(groupBy, '&groupBy=' + groupBy) + ifTrue(imageSize, '&imageSize=' + imageSize) + ifTrue(visitorIp, '&visitor_ip=' + visitorIp) + ifTrue(referrer, '&referrer=' + referrer) + ifTrue(userAgent, '&userAgent=' + userAgent) + ifTrue(relevancyThreshold, '&relevancyThreshold=' + relevancyThreshold) + ifTrue(filterCatalogId, '&filterCatalogId=' + convertArrayToString(filterCatalogId)) + ifTrue(filterCategory, '&filterCategory=' + convertArrayToString(filterCategory)) + ifTrue(filterKeyword, '&filterKeyword=' + convertArrayToString(filterKeyword)) + ifTrue(filterKeywords, '&filterKeywords=' + convertArrayToString(filterKeywords)) + ifTrue(filterPremier, '&filterPremier=' + filterPremier) + ifTrue(filterMerchant, '&filterMerchant=' + convertArrayToString(filterMerchant)) + ifTrue(filterMerchantId, '&filterMerchantId=' + convertArrayToString(filterMerchantId)) + ifTrue(filterProductId, '&filterProductId=' + convertArrayToString(filterProductId)) + ifTrue(maxPrice, '&maxPrice=' + toFixedNumber(maxPrice)) + ifTrue(minPrice, '&minPrice=' + toFixedNumber(minPrice)) + ifTrue(maxPriceSale, '&maxPriceSale=' + toFixedNumber(maxPriceSale)) + ifTrue(minPriceSale, '&minPriceSale=' + toFixedNumber(minPriceSale)) + ifTrue(limit, '&limit=' + limit) + ifTrue(page, '&page=' + page) + ifTrue(sid, '&sid=' + sid) + ifTrue(imageSize, '&imageSize=' + imageSize) + ifTrue(enableFacets, '&enableFacets=' + convertArrayToString(enableFacets)) + ifTrue(enableQuerySuggestion, '&enableQuerySuggestion=' + enableQuerySuggestion) + ifTrue(enableFullData, '&enableFullData=' + enableFullData) + ifTrue(debugMode, '&debugMode=' + debugMode) + ifTrue(imageMaskDomain, '&imageMaskDomain=' + imageMaskDomain) + ifTrue(clickMaskDomain, '&clickMaskDomain=' + clickMaskDomain) + ifTrue(imageSize, '&imageSize=' + imageSize), {
        // headers: {
